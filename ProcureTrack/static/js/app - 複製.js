...
function renderStatusSelect(current, rowId) {
    const options = (window.PROCURE_STATUS_OPTIONS || []).map(opt => {
...
function renderArrivalInput(value, rowId, cssClass = 'arrival-input') {
    ...
    return `<input type="date" class="form-control form-control-sm ${cssClass}" data-id="${escapeHtml(rowId)}" value="${v}" ${disabled}>`;
}
