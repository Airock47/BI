...
def check_permission(region):
    ...
    # Check Supervisor Status (Assuming 'V' based on user request, or '1', 'True')
    if is_supervisor == 'V':
        if user_dept == region:
            return True
...
@login_required
def index():
    data, month_labels, next_12_months, _ = get_psi_data()
...
def get_psi_data():
    ...
    # 5. Merging All Data
    all_products = set(df_inv['product_name']).union(set(df_sales_agg['product_name']))
    ...
    return df_master.to_dict('records'), month_labels, next_12_months, procure_details
