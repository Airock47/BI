...
def search():
    ...
    digit_count = sum(ch.isdigit() for ch in normalized_keyword)
    has_alpha = any(ch.isalpha() for ch in normalized_keyword)
    is_phone_query = digit_count >= 6 and not has_alpha
    phone_customer_codes = set()
    ...
    try:
        ...
        if is_phone_query:
            for row in customer_results:
                row_dict = dict(row)
                customer_code = row_dict.get(CUSTOMER_CODE_COL)
                if customer_code:
                    phone_customer_codes.add(customer_code)

            phone_queries = [
                (
                    f"SELECT DISTINCT {CUSTOMER_CODE_COL} FROM customer_basic WHERE {CUSTOMER_PHONE_COL} LIKE ?",
                    (search_pattern,),
                ),
                (
                    f"SELECT DISTINCT {CUSTOMER_CODE_COL} FROM customer_contacts WHERE {CUSTOMER_PHONE_COL} LIKE ? OR {NOTE_COL} LIKE ?",
                    (search_pattern, search_pattern),
                ),
                (
                    f"SELECT DISTINCT {CUSTOMER_CODE_COL} FROM customer_addresses WHERE {CUSTOMER_PHONE_COL} LIKE ? OR {NOTE_COL} LIKE ?",
                    (search_pattern, search_pattern),
                ),
            ]

            for sql, params in phone_queries:
                customer_cursor.execute(sql, params)
                for fetched in customer_cursor.fetchall():
                    code = fetched[CUSTOMER_CODE_COL]
                    if code:
                        phone_customer_codes.add(code)

            if phone_customer_codes:
                code_placeholders = ','.join(['?'] * len(phone_customer_codes))
                codes_tuple = tuple(phone_customer_codes)
                customer_cursor.execute(
                    f"""
                    SELECT 'customer' as type, *
                    FROM customer_basic
                    WHERE {CUSTOMER_CODE_COL} IN ({code_placeholders})
                    """,
                    codes_tuple,
                )
                additional_customers = customer_cursor.fetchall()
                for row in additional_customers:
                    results.append({
                        'type': 'customer',
                        'data': dict(row)
                    })
        ...
        if is_phone_query and phone_customer_codes:
            code_placeholders = ','.join(['?'] * len(phone_customer_codes))
            codes_tuple = tuple(phone_customer_codes)

            sales_conn = get_db_connection('sales.db')
            sales_cursor = sales_conn.cursor()
            sales_by_customer_query = f"""
            SELECT 'sales' as type, m.*
            FROM sales_main m
            WHERE m.{CUSTOMER_CODE_COL} IN ({code_placeholders})
            """
            sales_cursor.execute(sales_by_customer_query, codes_tuple)
            ...
            repair_conn = get_db_connection('repair.db')
            repair_cursor = repair_conn.cursor()
            repair_by_customer_query = f"""
            SELECT 'repair' as type, *
            FROM repair_data
            WHERE {CUSTOMER_CODE_COL} IN ({code_placeholders})
            """
            repair_cursor.execute(repair_by_customer_query, codes_tuple)
            for row in repair_cursor.fetchall():
                results.append({
                    'type': 'repair',
                    'data': dict(row)
                })
            ...
            custody_conn = get_db_connection('custody.db')
            custody_cursor = custody_conn.cursor()
            custody_by_customer_query = f"""
            SELECT 'custody' as type, *
            FROM custody_main
            WHERE {CUSTOMER_CODE_COL} IN ({code_placeholders})
            """
            custody_cursor.execute(custody_by_customer_query, codes_tuple)
            for row in custody_cursor.fetchall():
                results.append({
                    'type': 'custody',
                    'data': dict(row)
                })
